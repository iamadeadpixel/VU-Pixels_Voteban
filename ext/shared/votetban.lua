...
Events:Subscribe('Player:Chat', function(player, recipientMask, message)
    ...
    if s_Parts[1] == '!votetban' then
        ...
        if not Config.votetban then
            ChatManager:SendMessage("votetban is disabled")
            print("votetban disabled")
            return
        end

        if Config.votetban then
