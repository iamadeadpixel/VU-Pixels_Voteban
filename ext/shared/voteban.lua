...
Events:Subscribe('Player:Chat', function(player, recipientMask, message)
    ...
    if s_Parts[1] == '!voteban' then
        ...
        if not Config.voteban then
            ChatManager:SendMessage("voteban is disabled")
            print("voteban disabled")
            return
        end

        if Config.voteban then
